...
SOURCE_HTML = "converted_writeups/carpediem - htb hard (PHP file upload webshell, socks proxy network discovery, TLS weak algorithm traffic decryption, BackdropCMS module upload RCE, cron abuse privesc, host privesc from container CVE)/index.html"
...
4. **Content Enhancement**:
   ...
   - DO NOT omit content, only enhance it if needed, but never omit any content from the original writeup.
   - DO NOT omit explanations even if the techniques explained and/or used are not used in the exploit chain later. Respect the original content.
...
5. **Exploitation Summary**:
   ...
   - Do not omit details or explanations
   - DO USE paragraphs to avoid walls of text, I don't want more than 3-4 lines of text together as that's hard to read

6. **Formatting**:
   - Remove ALL timestamps from command outputs
   - Remove any user/root flag contents
   - Keep ALL original links intact, do not remove them
...
OUTPUT FORMAT:
Return ONLY valid, complete HTML code. No preamble, no conclusions section at the end, no mitigation steps, no final section with images - just the raw HTML that can be directly saved as index.html.
